...
//
// Copied from the VPNDetection Rust SDK rather than rewritten: the same reqwest
// blind spot produces the same seam, and two divergent recorders would be two
// things to debug.
...
/// It matters more here than it looks: the published catalog reaches several
/// gigabytes, and this proxy buffers whatever it forwards.
const MAX_BODY: usize = 32 << 20;

/// The most of an answer kept for a test to read afterwards. A database
/// transfer runs through this same code path, so a body is only ever remembered
/// when it is JSON and small.
