...
KEY_SECRET="INTERNETDATA_STAGING_KEY"
...
# characters then the next two. `internetdata` is therefore in/te/internetdata.
function indexPath() {
    local name="$1"
    case "${#name}" in
        1|2) echo "${#name}/${name}" ;;
...
# Names only, never values: these logs are public. Empty counts as absent, since
# CI interpolates a secret that does not exist to an empty string, so the
# variable is SET and a plain unset check never fires.
function reportKey() {
