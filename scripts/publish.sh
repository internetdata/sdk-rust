...
#
# The first release has to come through here, or through the release workflow's
# token fallback: crates.io attaches a trusted publisher to an EXISTING crate,
# so there is nothing to configure until the crate exists. Create the token at
# https://crates.io/settings/tokens with the publish-new scope; it needs no
...
docker run --rm \
    -v "$PWD:/src:ro" \
    -v internetdata-rust-target:/target \
