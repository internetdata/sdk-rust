...
async fn no_oauth_request_carries_the_api_key() {
    ...
    let client = stub.anonymous().api_key(&data.no_credential.api_key).build().expect("build");
...
async fn every_oauth_call_takes_a_per_call_timeout() {
    let stub = Stub::start(every_path(Route::ok(EVERY_REQUIRED_MEMBER).waiting(STALL))).await;
    let client = stub.anonymous().retries(0).timeout(CLIENT_TIMEOUT).build().expect("build");
    let oauth = client.oauth();
...
async fn serving(served: &corpus::Served) -> Client {
    ...
    stub.anonymous().retries(0).build().expect("build")
}
