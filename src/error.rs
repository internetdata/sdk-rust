...
pub enum Error {
    ...
    #[error("internetdata: {kind} (HTTP {status}): {message}")]
    Api {
        kind: ErrorKind,
        /// The API's own `rc`, which names the refusal.
        message: String,
        status: u16,
    ...
    #[error("internetdata: network: {0}")]
    Network(#[from] reqwest::Error),

    /// A database transfer failed on this side of the socket: the stream ended
    /// before the promised length (`UnexpectedEof`), the disk filled, or the
    /// destination could not be written. The inner [`std::io::ErrorKind`] is
    /// what tells those apart.
    #[error("internetdata: io: {0}")]
    Io(#[from] std::io::Error),

    /// The client was asked for something it cannot do, before any request was
    /// sent: an unusable base URL, for instance.
    #[error("internetdata: {0}")]
    Config(String),
}
...
impl Error {
    ...
    /// The API's own `rc`, or the transport failure's text.
    ...
    pub(crate) fn from_response(status: u16, retry_after: Option<Duration>, body: &str) -> Self {
        let message =
            envelope_rc(body).unwrap_or_else(|| format!("request failed with status {status}"));
...
pub(crate) fn kind_for_status(status: u16, retry_after: Option<Duration>) -> ErrorKind {
    match status {
        ...
        // default would make it retryable, so a misspelled database id would be
        // retried twice before failing. Three of the four VPNDetection SDKs
        // shipped exactly that. Classify on the RANGE, not on an enumerated
        // list.
...
/// The API refuses with `{"rc": "..."}` on every endpoint, so the code it names
/// is what a caller is told rather than a status invented here.
fn envelope_rc(body: &str) -> Option<String> {
    #[derive(Deserialize)]
    struct Envelope {
        rc: Option<String>,
    }
    let envelope: Envelope = serde_json::from_str(body).ok()?;
    envelope.rc.filter(|rc| !rc.is_empty())
}
