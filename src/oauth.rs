...
impl<'a> OauthApi<'a> {
    ...
    pub(crate) async fn poll<C: PollClock + Sync>(
        ...
        loop {
            ...
            // full, `interval` 2147483647 held a poll with 2 s left for 68 years,
            // and a `slow_down` 5 s past it (2.2.1, measured 2026-09-26).
...
impl OauthOptions {
    ...
    /// request, never the poll as a whole. Zero is refused as a bad request
    /// before anything is sent, the poll's before its first wait.
...
impl DeviceAuthorizationOptions {
    ...
    /// Overrides [`crate::ClientBuilder::timeout`] for this call, still per
    /// attempt. Zero is refused as a bad request before anything is sent.
    pub fn timeout(mut self, timeout: Duration) -> Self {
        self.timeout = Some(timeout);
        self
    }
...
impl std::fmt::Display for OauthError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match (self, self.status()) {
            ...
            (_, Some(status)) => {
                write!(f, "internetdata: oauth (HTTP {status}): {}", self.message())
            }
            (_, None) => write!(f, "internetdata: oauth: {}", self.message()),
        }
    }
}
