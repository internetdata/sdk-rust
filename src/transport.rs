...
/// Every request the crate makes: five database GETs with no request bodies,
/// and the OAuth requests, which never carry the key.
///
/// The generated client is not used for this: its `ResponseContent` carries no
/// headers, so a 429's `Retry-After` is unreachable, and its
/// `download_database_v2` is typed `Result<(), _>`, discarding the 302 whose
/// `Location` is that endpoint's entire answer.
#[derive(Debug, Clone)]
...
impl Transport {
    ...
    /// behind it reaches gigabytes, so following it would transfer the whole
    /// file to hand back a link. [`crate::ClientBuilder`] builds its client with
    /// `redirect::Policy::none()` for exactly this; a caller-supplied client
    ...
    ///
    /// The body is handed back unread, so a database of any size is streamed by
